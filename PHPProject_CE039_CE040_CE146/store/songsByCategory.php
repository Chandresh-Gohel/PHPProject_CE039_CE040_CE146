...
if (isset($_POST["filter_by_category"])) {
  $category_id = $_POST['filter_by_category'];  
} else {
  $category_id = "";
}
...
<div class="container">
  ...
  <form class="form-inline" method="POST" action="songsByCategory.php">
            ...
            <option value="CATEGORY" selected="selected">SELECT CATEGORY</option>
                ...
                ?>
                <option name='filter_by_category' id='filter_by_category' value="<?php echo $rowCategories['id'] ?>" length="90px" value="<?php echo $rowCategories['catname']?>"><?php echo $rowCategories['catname']?></option>
                <?php
              $rowCategories=$getcategories->fetch(PDO::FETCH_ASSOC); 
                  }
